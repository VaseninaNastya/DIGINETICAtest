(function() {
	...
	var multirange = function(input) {
		...
		function update(mode) {
			ghost.style.setProperty("--low", 100 * ((input.valueLow - min) / (max - min)) + "%");
			ghost.style.setProperty("--high", 100 * ((input.valueHigh - min) / (max - min)) + "%");
			document.querySelector('#price_input__start').setAttribute('value',((input.valueLow - min))* maxValue/ (max - min) )
			document.querySelector('#price_input__end').setAttribute('value',((input.valueHigh - min))* maxValue/ (max - min) )
